    ...
    @section('content')
        ...
        <div class="admin-container py-5">
            <x-admin-breadcrumb :items="[['name' => 'إدارة الجهات ']]" />
            <!-- رأس الصفحة -->
                    ...
                    <tbody>
                        @forelse($governments as $gov)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('admin.government-offers.index', $gov->id) }}"
                                        class="btn btn-sm btn-outline-info" title="إدارة العروض"
                                        style="padding: 4px 12px; border-radius: 20px; font-size: 0.7rem; display: inline-flex; align-items: center; gap: 4px;">
                                        <i class="fas fa-gift"></i>
                                        <span>عروض</span>
                                    </a>
                                    <button class="btn-edit"
                                        onclick='editGovernment({{ json_encode($gov->load('services')) }})'>
                                        <i class="fas fa-edit"></i>
                                        <span>تعديل</span>
                                    </button>
                                    <button class="btn-delete"
                                        onclick='deleteGovernment({{ $gov->id }}, "{{ $gov->name }}")'>
        ...
        <div id="addModal" class="modal">
            <div class="modal-content">
                <form id="addGovernmentForm" method="POST" action="{{ route('admin.governments.store') }}"
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="search_address" id="add_search_address" class="form-control"
                                placeholder="مثال: مستشفى الكويت، صنعاء، اليمن">
                        ...
                        {{-- ساعات العمل - نسخة محسنة --}}
                        <div class="form-group">
                            <label class="form-label">ساعات العمل</label>

                            {{-- خيار 24 ساعة --}}
                            <div class="form-check mb-3">
                                <input type="checkbox" name="is_24h" id="add_is_24h" class="form-check-input"
                                    value="1">
                                <label class="form-check-label" for="add_is_24h">
                                    <i class="fas fa-clock"></i> مفتوح 24 ساعة (طوال الأسبوع)
                                </label>
                            </div>

                            {{-- منشئ الساعات --}}
                            <div id="addWorkHoursBuilder" style="display: block;">
                                <div class="row g-2">
                                    @php
                                        $days = [
                                            'saturday' => 'السبت',
                                    ...
                                    @foreach ($days as $dayKey => $dayName)
                                        <div class="col-md-6 col-lg-4">
                                            <div class="border rounded-3 p-2 mb-2 bg-light">
                                                <div class="fw-bold mb-2 text-primary">{{ $dayName }}</div>
                                                <div class="row g-1">
                                                    <div class="col-6">
                                                        <label class="small text-muted">من</label>
                                                        <input type="time" name="hours[{{ $dayKey }}][open]"
                                                            class="form-control form-control-sm">
                                                    </div>
                                                    <div class="col-6">
                                                        <label class="small text-muted">إلى</label>
                                                        <input type="time" name="hours[{{ $dayKey }}][close]"
                                                            class="form-control form-control-sm">
                                                    </div>
                                                </div>
                                            </div>
                                ...
                                <small class="text-muted">اترك الحقول فارغة للأيام التي لا تعمل فيها الجهة</small>
                            </div>

                            {{-- حقل مخفي لتخزين JSON --}}
                            <input type="hidden" name="work_hours_json" id="add_work_hours_json">
                        </div>

                        <script>
                            // بناء JSON لساعات العمل عند إرسال نموذج الإضافة
                            function buildAddWorkHoursJSON() {
                                const is24h = document.getElementById('add_is_24h')?.checked || false;
                                let workHours = {
                                    is_24h: is24h
                                };
                                ...
                                if (!is24h) {
                                    ...
                                    days.forEach(day => {
                                        ...
                                        // ✅ أضف اليوم فقط إذا كان كلا الحقلين ممتلئين
                                ...
                                document.getElementById('add_work_hours_json').value = JSON.stringify(workHours);
                                console.log('Work Hours JSON:', workHours); // للتأكد
                                return true;
                            }

                            // إظهار/إخفاء منشئ الساعات عند تغيير خيار 24 ساعة
                            document.getElementById('add_is_24h')?.addEventListener('change', function() {
                                const builder = document.getElementById('addWorkHoursBuilder');
                                if (builder) {
                                    builder.style.display = this.checked ? 'none' : 'block';
                                }
                            });

                            // ربط الحدث للنموذج
                            document.getElementById('addGovernmentForm')?.addEventListener('submit', function(e) {
                                buildAddWorkHoursJSON();
                            });
                        </script>
                        <div class="row">
                            <div class="col-md-6">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="whatsapp_number" class="form-control"
                                        placeholder="مثال: 712345678">
                                </div>
                            </div>
                        </div>

                        <div class="form-group">
                            <label class="form-label">وصف الموقع التفصيلي</label>
                            <textarea name="location_description" class="form-control" rows="2"
                                placeholder="مثال: بجوار حديقة السبعين، بعد الإشارة بحوالي 200 متر"></textarea>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label class="form-label">رابط تليجرام</label>
                        ...
                        <div class="form-group">
                            <label class="form-label">كلمات مفتاحية (للبحث)</label>
                            <input type="text" name="keywords" class="form-control"
                                placeholder="كلمات مفصولة بفواصل، مثال: مستشفى, طوارئ, قلب">
                            <small class="text-muted">تساعد في تحسين نتائج البحث</small>
                        </div>

                        <div class="form-group">
                            <label class="form-label">وصف قصير (للبطاقة الرئيسية)</label>
                            <textarea name="short_description" class="form-control" rows="2"
                                placeholder="وصف مختصر للجهة يظهر في البطاقة الرئيسية"></textarea>
                        </div>
                        <div class="form-group">
                ...
                <form id="editForm" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="search_address" id="edit_search_address" class="form-control"
                                placeholder="مثال: مستشفى الكويت، صنعاء، اليمن">
                        ...
                        {{-- ساعات العمل - نسخة محسنة للتعديل --}}
                        <div class="form-group">
                            <label class="form-label">ساعات العمل</label>

                            <div class="form-check mb-3">
                                <input type="checkbox" name="is_24h" id="edit_is_24h" class="form-check-input"
                                    value="1"
                                    {{ isset($government) && isset($government->work_hours_json['is_24h']) && $government->work_hours_json['is_24h'] === true ? 'checked' : '' }}>
                                <label class="form-check-label" for="edit_is_24h">
                                    <i class="fas fa-clock"></i> مفتوح 24 ساعة (طوال الأسبوع)
                                </label>
                            </div>

                            <div id="editWorkHoursBuilder"
                                style="{{ isset($government) && isset($government->work_hours_json['is_24h']) && $government->work_hours_json['is_24h'] === true ? 'display: none;' : 'display: block;' }}">
                                <div class="row g-2">
                                    @php
                                        $days = [
                                            'saturday' => 'السبت',
                                        ...
                                        $currentHours =
                                            isset($government) && $government->work_hours_json
                                                ? $government->work_hours_json
                                                : [];
                                        if (isset($currentHours['is_24h'])) {
                                            unset($currentHours['is_24h']);
                                        }
                                    @endphp

                                    @foreach ($days as $dayKey => $dayName)
                                        @php $dayData = $currentHours[$dayKey] ?? ['open' => '', 'close' => '']; @endphp
                                        <div class="col-md-6 col-lg-4">
                                            <div class="border rounded-3 p-2 mb-2 bg-light">
                                                <div class="fw-bold mb-2 text-primary">{{ $dayName }}</div>
                                                <div class="row g-1">
                            ...
                            <input type="hidden" name="work_hours_json" id="edit_work_hours_json">
                        </div>

                        <script>
                            function buildEditWorkHoursJSON() {
                                const is24h = document.getElementById('edit_is_24h')?.checked || false;
                                let workHours = {
                                    is_24h: is24h
                                };
                                ...
                                if (!is24h) {
                                    ...
                                    days.forEach(day => {
                                        const openInput = document.querySelector(`#editModal input[name="hours[${day}][open]"]`);
                                        const closeInput = document.querySelector(`#editModal input[name="hours[${day}][close]"]`);

                                        if (openInput && closeInput) {
                                            const open = openInput.value;
                                            const close = closeInput.value;

                                            console.log(`${day}: open="${open}", close="${close}"`);

                                            if (open && open.trim() !== '' && close && close.trim() !== '') {
                                                workHours[day] = {
                                                    open: open,
                                                    close: close
                                                };
                                ...
                                document.getElementById('edit_work_hours_json').value = JSON.stringify(workHours);
                                return true;
                            }

                            // ✅ المستمعات المضافة
                            document.getElementById('edit_is_24h')?.addEventListener('change', function() {
                                const builder = document.getElementById('editWorkHoursBuilder');
                                if (builder) {
                                    builder.style.display = this.checked ? 'none' : 'block';
                                }
                            });
                        ...
                        <div class="form-group">
                            <label class="form-label">وصف الموقع التفصيلي</label>
                            <textarea name="location_description" id="edit_location_description" class="form-control" rows="2"></textarea>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label class="form-label">رابط فيسبوك</label>
                                    <input type="url" name="facebook_url" id="edit_facebook_url"
                                        class="form-control">
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="form-group">
                                    <label class="form-label">رابط تليجرام</label>
                                    <input type="url" name="telegram_url" id="edit_telegram_url"
                                        class="form-control">
                                </div>
                            </div>
                        </div>
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <input type="text" id="serviceSearchInput" class="form-control"
                            placeholder="اكتب اسم الخدمة للبحث..." autocomplete="off" onkeyup="filterServices()"
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" id="serviceContactNumber" class="form-control"
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="form-label">ساعات العمل</label>
                                <input type="text" id="serviceWorkHours" class="form-control"
        ...
        <script>
            ...
            async function fetchLocationFromAddress(address, type = 'add') {
                const hintDiv = type === 'add' ? document.getElementById('add_location_hint') : document.getElementById(
                    'edit_location_hint');
                ...
                if (hintDiv) {
                    hintDiv.innerHTML =
                        '<span class="location-loading"><i class="fas fa-spinner fa-spin"></i> جاري البحث عن الموقع...</span>';
                }
                ...
                try {
                    ...
                    if (data.success && data.lat && data.lng) {
                        ...
                        if (hintDiv) {
                            hintDiv.innerHTML =
                                `<span class="location-success"><i class="fas fa-check-circle"></i> ✓ تم العثور على الموقع: ${(data.address || address).substring(0, 100)}</span>`;
                            setTimeout(() => {
                    ...
                    if (hintDiv) {
                        hintDiv.innerHTML =
                            '<span class="location-error"><i class="fas fa-exclamation-triangle"></i> لم يتم العثور على الموقع. جرب: صنعاء، اليمن أو مستشفى الثورة، صنعاء</span>';
                        setTimeout(() => {
            ...
            function editGovernment(gov) {
                ...
                document.getElementById('edit_email').value = gov.email || '';
                document.getElementById('edit_whatsapp_number').value = gov.whatsapp_number || '';
                document.getElementById('edit_location_description').value = gov.location_description || '';
                document.getElementById('edit_facebook_url').value = gov.facebook_url || '';
                document.getElementById('edit_telegram_url').value = gov.telegram_url || '';
                document.getElementById('edit_keywords').value = gov.keywords || '';
                document.getElementById('edit_short_description').value = gov.short_description || '';

                // تعبئة حقول الموقع المخفية
                ...
                if (editHintDiv) {
                    if (gov.address) {
                        editHintDiv.innerHTML =
                            `<span class="location-success"><i class="fas fa-info-circle"></i> 📍 الموقع الحالي: ${gov.address.substring(0, 100)}${gov.address.length > 100 ? '...' : ''}</span>`;
                    } else {
                        editHintDiv.innerHTML = '';
                    }
                }

                // ===== تعبئة ساعات العمل (النظام الجديد) =====
                // تعبئة خيار 24 ساعة
                const is24hCheckbox = document.getElementById('edit_is_24h');
                if (is24hCheckbox) {
                    const is24h = gov.work_hours_json && gov.work_hours_json.is_24h === true;
                    is24hCheckbox.checked = is24h;

                    // إظهار/إخفاء منشئ الساعات
                    const builder = document.getElementById('editWorkHoursBuilder');
                    if (builder) {
                        builder.style.display = is24h ? 'none' : 'block';
                    }
                }

                // تعبئة ساعات كل يوم
                if (gov.work_hours_json && !gov.work_hours_json.is_24h) {
                    const days = ['saturday', 'sunday', 'monday', 'tuesday', 'wednesday', 'thursday', 'friday'];
                    days.forEach(day => {
                        const openInput = document.querySelector(`input[name="hours[${day}][open]"]`);
                        const closeInput = document.querySelector(`input[name="hours[${day}][close]"]`);
                        if (openInput && closeInput && gov.work_hours_json[day]) {
                            openInput.value = gov.work_hours_json[day].open || '';
                            closeInput.value = gov.work_hours_json[day].close || '';
                        }
                    });
                }
                ...
                if (governmentServices.length === 0) {
                    ...
                    governmentServices.forEach((service, idx) => {
                        const pivot = service.pivot || {};
                        const uniqueId = `edit_service_${service.id}_${idx}`;
                        const isHospital = categoryId == 1;
                        ...
                        <div class="service-details-fields" id="details_${uniqueId}" style="display: none;">
                            ...
                            <div class="row g-2 mt-2 pt-2 border-top">
                                <div class="col-md-6">
                                    ...
                                    <input type="text" name="services[${service.id}][processing_time]" class="form-control form-control-sm" value="${escapeHtml(pivot.processing_time || '')}" placeholder="مثال: 3 أيام، أسبوع">
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label small">📍 موقع التقديم</label>
                                    <input type="text" name="services[${service.id}][office_location]" class="form-control form-control-sm" value="${escapeHtml(pivot.office_location || '')}" placeholder="مثال: شباك 5، الدور الثاني">
                                </div>
                                <div class="col-12">
                                    <label class="form-label small">📄 الأوراق المطلوبة</label>
                                    <textarea name="services[${service.id}][required_documents]" class="form-control form-control-sm" rows="3">${escapeHtml(pivot.required_documents || '')}</textarea>
                                </div>
                                <div class="col-12">
                                    <label class="form-label small">📋 الإجراءات خطوة بخطوة</label>
                                    <textarea name="services[${service.id}][steps]" class="form-control form-control-sm" rows="3">${escapeHtml(pivot.steps || '')}</textarea>
                                </div>
                                <div class="col-12">
                                    <label class="form-label small">📜 الشروط</label>
                                    <textarea name="services[${service.id}][conditions]" class="form-control form-control-sm" rows="2">${escapeHtml(pivot.conditions || '')}</textarea>
                                </div>
                                <div class="col-12">
                                    <label class="form-label small">📝 ملاحظات إضافية</label>
                                    <textarea name="services[${service.id}][notes]" class="form-control form-control-sm" rows="2">${escapeHtml(pivot.notes || '')}</textarea>
                                </div>
                                <div class="col-6">
                                    <div class="form-check mt-2">
                                        <input type="checkbox" name="services[${service.id}][requires_appointment]" class="form-check-input" id="appointment_edit_${service.id}" value="1" ${pivot.requires_appointment ? 'checked' : ''}>
                                        <label class="form-check-label small" for="appointment_edit_${service.id}">📅 يتطلب حجز مسبق</label>
                                    </div>
                                </div>
                                <div class="col-6">
                                    <label class="form-label small">📞 رقم الحجز</label>
                                    <input type="text" name="services[${service.id}][appointment_phone]" class="form-control form-control-sm" value="${escapeHtml(pivot.appointment_phone || '')}">
                                </div>
                            </div>
                ...
                filteredServices.forEach(service => {
                    ...
                    if (index !== -1) {
                        highlightedName = name.substring(0, index) +
                            '<strong style="color: #2f3e9e;">' + name.substring(index, index + searchTerm.length) +
                            '</strong>' +
            ...
            function renderAddedServices() {
                ...
                tempServices.forEach((service, index) => {
                    const uniqueId = `service_${service.id}_${index}`;
                    html += `
            <div class="service-card card p-3 mb-3" data-service-id="${service.id}">
                <div class="d-flex justify-content-between align-items-center mb-2">
                    <div class="d-flex align-items-center gap-2">
                ...
                <div class="service-details-fields" id="details_${uniqueId}" style="display: none;">
                    ...
                    <div class="row g-2 mt-2 pt-2 border-top">
                        <div class="col-md-6">
                            <label class="form-label small">⏱️ مدة الإنجاز</label>
                            <input type="text" name="services[${service.id}][processing_time]" class="form-control form-control-sm" value="${escapeHtml(service.processing_time || '')}" placeholder="مثال: 3 أيام، أسبوع">
                        </div>
                        <div class="col-md-6">
                            <label class="form-label small">📍 موقع التقديم</label>
                            <input type="text" name="services[${service.id}][office_location]" class="form-control form-control-sm" value="${escapeHtml(service.office_location || '')}" placeholder="مثال: شباك 5، الدور الثاني">
                        </div>
                        <div class="col-12">
                            <label class="form-label small">📄 الأوراق المطلوبة</label>
                            <textarea name="services[${service.id}][required_documents]" class="form-control form-control-sm" rows="3" placeholder="• أصل قيد عائلي&#10;• صورتان شخصيتان">${escapeHtml(service.required_documents || '')}</textarea>
                        </div>
                        <div class="col-12">
                            <label class="form-label small">📋 الإجراءات خطوة بخطوة</label>
                            <textarea name="services[${service.id}][steps]" class="form-control form-control-sm" rows="3" placeholder="1. تعبئة النموذج&#10;2. دفع الرسم">${escapeHtml(service.steps || '')}</textarea>
                        </div>
                        <div class="col-12">
                            <label class="form-label small">📜 الشروط</label>
                            <textarea name="services[${service.id}][conditions]" class="form-control form-control-sm" rows="2">${escapeHtml(service.conditions || '')}</textarea>
                        </div>
                        <div class="col-12">
                            <label class="form-label small">📝 ملاحظات إضافية</label>
                            <textarea name="services[${service.id}][notes]" class="form-control form-control-sm" rows="2">${escapeHtml(service.notes || '')}</textarea>
                        </div>
                        <div class="col-6">
                            <div class="form-check mt-2">
                                <input type="checkbox" name="services[${service.id}][requires_appointment]" class="form-check-input" id="appointment_add_${service.id}" value="1" ${service.requires_appointment ? 'checked' : ''}>
                                <label class="form-check-label small" for="appointment_add_${service.id}">📅 يتطلب حجز مسبق</label>
                            </div>
                        </div>
                        <div class="col-6">
                            <label class="form-label small">📞 رقم الحجز</label>
                            <input type="text" name="services[${service.id}][appointment_phone]" class="form-control form-control-sm" value="${escapeHtml(service.appointment_phone || '')}">
                        </div>
                    </div>
                </div>
            </div>
            ...
            document.getElementById('addImagesInput')?.addEventListener('change', function(e) {
                ...
                files.forEach(file => {
                    ...
                    reader.onload = function(event) {
                        ...
                        col.className = 'col-3';
                        col.innerHTML =
                            `<img src="${event.target.result}" class="img-fluid img-preview" style="height: 100px; object-fit: cover;">`;
            ...
            (function() {
                ...
                if (sessionStorage.getItem('lastListPage') && window.location.href === sessionStorage.getItem(
                        'lastListPage')) {
                    if (window.history.replaceState) {
                        window.history.replaceState(null, null, window.location.href);
                    }
