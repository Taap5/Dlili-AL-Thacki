    ...
    <div class="d-flex flex-wrap justify-content-between align-items-center mb-4 gap-3">
        ...
        <div class="dropdown">
            ...
            <ul class="dropdown-menu">
                <li><a class="dropdown-item" href="{{ route('governments.index') }}">الكل</a></li>
                @foreach($categories as $cat)
                    <li><a class="dropdown-item" href="{{ route('governments.index', ['category' => $cat->id]) }}">{{ $cat->name }}</a></li>
                @endforeach
            </ul>
        </div>
    </div>
                    ...
                    <div class="card government-card h-100 border-0 shadow-sm rounded-4 overflow-hidden">
                        ...
                        <div class="card-body p-4">
                            ...
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                ...
                                <!-- التصنيف -->
                                <span class="badge bg-primary">{{ $gov->category->name ?? 'غير مصنف' }}</span>
                            </div>
                            ...
                            @if($gov->contact_number || $gov->work_hours)
                                <div class="small text-muted">
                                    @if($gov->contact_number)
                                        <i class="fas fa-phone-alt me-1"></i> {{ $gov->contact_number }}
                                    @endif
                                    @if($gov->work_hours)
                                        <div class="mt-1"><i class="fas fa-clock me-1"></i> {{ $gov->work_hours }}</div>
                                    @endif
                                </div>
                            @endif
                        </div>
    ...
    @else
        <div class="card border-0 shadow-sm rounded-4 bg-light">
            <div class="card-body text-center py-5">
                ...
                <h5 class="text-muted">لا توجد جهات حكومية</h5>
