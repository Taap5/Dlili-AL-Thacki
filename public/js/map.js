document.addEventListener("DOMContentLoaded", function () {
    ...
    directionBtns.forEach(btn => {
        btn.addEventListener("click", function() {
            directionBtns.forEach(b => b.classList.remove("direction-active"));
            this.classList.add("direction-active");
            selectedProfile = this.getAttribute("data-profile") || "driving-car";
...
window.calculateRouteAndDraw = function(startLat, startLng, endLat, endLng, profile = "driving-car") {
    if (!window.mapInstance) return;

    // استخدام دالة calculateRoute من map.js
    if (typeof calculateRoute === 'undefined') {
        // تعريف دالة calculateRoute إذا لم تكن موجودة
        const ORS_API_KEY = "5b3ce3597851110001cf6248e9b2c8f95d32449f86b7b3b1f3e2b3e1";

        window.calculateRoute = function(userLat, userLng, govLat, govLng, profile = "driving-car") {
            const url = `https://api.openrouteservice.org/v2/directions/${profile}?api_key=${ORS_API_KEY}&start=${userLng},${userLat}&end=${govLng},${govLat}`;
            return fetch(url)
                .then(res => res.json())
                .then(data => {
        ...
        window.drawRoute = function(coords) {
            if (window.routeLayer) window.mapInstance.removeLayer(window.routeLayer);
    ...
    // حساب المسار
    return window.calculateRoute(startLat, startLng, endLat, endLng, profile)
        .then(res => {
...
function drawStraightLineFallback(startLat, startLng, endLat, endLng) {
    ...
    if (routeInfo) {
        routeInfo.classList.remove('d-none');
        document.getElementById('distanceText').innerText = distance.toFixed(2) + " كم";
